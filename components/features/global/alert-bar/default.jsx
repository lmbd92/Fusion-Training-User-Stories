...
export default function AlertBar() {
  ...
  const { alertType, link, linkText, text } = componentContext.customFields;
  ...
  return (
    <section className={`alert alert-${alertType} w-100`} role="alert">
      <span {...editableField("text")} suppressContentEditableWarning>
        {text}
      </span>
      {linkText && (
        <Fragment>
...
AlertBar.propTypes = {
  customFields: PropTypes.shape({
    alertType: PropTypes.oneOf(["danger", "warning", "info"]).tag({
      defaultValue: "danger",
      labels: {
        danger: "breaking",
        warning: "warning",
